...
The \textit{discriminant power} is calculated of each gene included in the classifier (the \textit{@classificationGenes}) when building the classifier (\textit{geNetClassifier()}). The \textit{plotDiscriminantPower()} function is included in the package to generate a graphic representation of the \textit{discriminant power}. 
