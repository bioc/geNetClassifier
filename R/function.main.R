...
{	
	...
	if(class(sampleLabels) != "factor") { warning("The argument 'classification sampleLabels' had to be converted into a factor.", immediate. = TRUE)}	
	...
	numElemClass <-  numSamples / numClasses 	
	...
	{
		warning("In order to remove correlations or interactions, the genes network needs to be calculated.", immediate. = TRUE)
		calculateNetwork<-TRUE
	}
	if(calculateNetwork && !is.null(genesNetwork)) 
	{
		warning("The genes network was provided. It will not be recalculated.", immediate. = TRUE)
	...
	{
		if (!is.null(genesRankingGlobal)) warning("The genesRanking provided is not valid. It will be re-calculated.", immediate. = TRUE)
	...
	}else
	{
		if (!any(gClasses(genesRankingGlobal) %in% classes)) # The genes ranking and the classes given sould be in the same order? 	
		{
			...
			{
				genesRankingGlobal <- NULL
				warning("The genesRanking given as argument does not match the expression matrix. It will be re-calculated.", immediate. = TRUE)
			}
		}
	}
	...
	if((numElemClass - trunc(numElemClass)) !=0) stop("There should be the same number of samples in each class.")
	if (sum(table(sampleLabels)!=numElemClass)!=0) stop("There should be the same number of samples in each class.")  
	if(estimateGError &&(numSamples%%5)!=0) warning("Since the number of samples is not multiple of 5, some samples might be used as test in several cross-validation loops when estimating the generalization error of the classifier.", immediate. = TRUE)
	...
	esetFiltered <- esetFiltered[, indexes, drop=F]
	...
	# Default lpThreshold= 0.75. Loss of pvalue significance:25perCent
	...
	meanExprDiff<- difMean(esetFiltered[topGenes,], numClasses, numElemClass)
	colnames(meanExprDiff) <- classes
	...
	{	
		...
		numSamplesCV <- kfoldcv(numCV.extern, numSamples)
		sampleOVR_CV <- sampeling(numElemClass, numClasses)
	...
	if(buildClassifier || estimateGError)
	{
		...
		for( i in 1:numCV.total)		 
		{
			...
			}else #if(estimateGError)
			{
				# Divide samples in Train and Test
				if( i == 1) {
					testSamples <- sampleOVR_CV[1:numSamplesCV[1]]
					trainSamples <- sort(sampleOVR_CV[-(1:numSamplesCV[1])])
				}else 
				{
					testSamples <- sampleOVR_CV[(sum(numSamplesCV[1:(i-1)]) + 1):sum(numSamplesCV[1:i])]
					trainSamples <- sort(sampleOVR_CV[-((sum(numSamplesCV[1:(i-1)]) + 1):sum(numSamplesCV[1:i]))])
				}
			...
			ifelse(dim(genesRankingLoop@ord)[2]==1, colnames(numGenesClass)<-"BothClasses", colnames(numGenesClass)<- gClasses(genesRankingLoop))
			
			for(k in 1:numIters)
			{
				numElemClassTrain <- length(trainSamples)/numClasses
				sampleOVR <- sampeling(numElemClassTrain, numClasses)
				
				# Select initial number of genes (minGenesTrain) from all the classes
				if (is.null(genesRedundancy))
				{
					 genes <- getRanking(genesRankingLoop, showGeneID=TRUE)$geneID[1:minGenesTrain,, drop=F] # En vez de minGenesTrainLoop, se quitan los NA despues
				} else 
				{
				...
				trainEset <- esetFiltered[genes,trainSamples, drop=FALSE]
				if(is.null(rownames(trainEset))) rownames(trainEset) <- genes
				geneSelectClassifier <- linear.SVM(trainEset, sampleLabels[trainSamples], numClasses, sampleOVR, numCV.intern, minProbAssignCoeff=minProbAssignCoeff, minDiffAssignCoeff=minDiffAssignCoeff)
				numGenesTP <- rbind(numGenesTP, c(numGenesClass[k,], sum(diag(geneSelectClassifier$confMatrix))/(numElemClassTrain*numClasses)))
				...
				while((any(numGenes2add< maxGenesTrain) && any(which(numGenes2add < maxGenesTrain) %in% worstClasses)) &&  (any(diag(geneSelectClassifier$confMatrix)< numElemClassTrain) || continueZeroError)) 
				{
					...
					if (numClasses > 2 ) 
					{
						...
						{
							worstClasses <- which(diag(geneSelectClassifier$confMatrix) != numElemClassTrain) #V1: Classes which don't have all in the diagonal
					...
					if(length(worstClasses)>0) # initialized: worstClasses <- 1:numClasses
					{
						...
						geneSelectClassifier <- linear.SVM(esetFiltered[genes, trainSamples], sampleLabels[trainSamples], numClasses, sampleOVR, numCV.intern) # V4: Usando NA tb internamente
						# Take note of its error
						numGenesTP[nrow(numGenesTP),"TruePositives"] <- sum(diag(geneSelectClassifier$confMatrix))/(numElemClassTrain*numClasses)
					}
				}
			...
			if (max(numTrainGenes[i,]) > dim(bestGenes)[1])
			{
				...
				warning ("There arent enough genes to build the classifier with the minimum error.", immediate. = TRUE)
			...
			# Train classifier
			finalClassifier  <-  svm(x=t(esetFilteredDataFrame[buildGenesVector, trainSamples]), y=sampleLabels[trainSamples], C=1, kernel="linear", probability=T )
		...
		{
			warning("The maximum numbers of genes passed as argument is bigger than the number of available genes in some of the internal loops. These were used instead:", immediate. = TRUE)
	...
	{	
		...
		generalizationError  <- new("GeneralizationError", accuracy=confusionMatrixStats$global, sensitivitySpecificity=confusionMatrixStats$byClass, confMatrix=mxcf, probMatrix=probMatrix, querySummary=predictionStats, classificationGenes.stats=genesStats, classificationGenes.num=numTrainGenes[1:numCV.extern,,drop=F] )
	}
	...
	if(buildClassifier) 
	{
		for(cl in 1:dim(trainGenes)[2])
		{
			...
			if(length(clGenes)>0)
			{
				tempDP <- t(sapply(trainGenes[clGenes,cl, drop=F], function(x) SV.dif(finalClassifier, x, originalGeneNames=rownames(esetFiltered))))

				tempDP <- data.frame(discriminantPower= tempDP[,"discriminantPower", drop=F], discrPwClass= tempDP[,"discrPwClass",drop=F])
	...
	if(buildClassifier && verbose)
	{
		...
		message(paste("\nClassifier trained with ",numSamples," samples. (",numElemClass," samples for each of the ",numClasses," classes)",sep=""))
	...
	if(buildClassifier || estimateGError)
	{
		...
		for(i in 1: length(lp))
		{
			if(any(numTrainGenes[,i]> lp[i]))
			{
					if(firstWarning)
					{
						warning (paste("For these classes, it was needed to take some genes with posterior probability under the ", lpThreshold," threshold:",sep=""), immediate. = TRUE)
						firstWarning<-FALSE
					}
